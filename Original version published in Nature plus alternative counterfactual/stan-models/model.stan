...
generated quantities {
    ...
    
    {
      ...
      for (m in 1:M){
        ...
        if (m != 10) {
          Rt_cf[,m] = mu[m] * exp(-X[m][,5] * (sum(append_row(segment(alpha, 2, 3), alpha[6])) + last_intervention[10]));
        } else {
          Rt_cf[,m] = mu[m] * exp(-X[m][,1:6] * alpha[1:6] - X[m][,5] * lockdown[m]- X[m][,7] * last_intervention[m]);
        }
