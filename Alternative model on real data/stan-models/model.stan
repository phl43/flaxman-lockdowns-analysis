...
parameters {
  real<lower=0> mu[M];
  real<lower=0,upper=mu[M]> omega[M];
  real<upper=0> rho[M];
...
transformed parameters {
    ...
    
    {
      ...
      for (m in 1:M){
        ...
        for (i in (N0+1):N2) {
          Rt[i,m] = mu[m] - 1 / (1 + exp(rho[m] * (i - theta[m]))) * (mu[m] - omega[m]);
          convolution = dot_product(sub_col(prediction, 1, m, i-1), tail(SI_rev, i-1));
