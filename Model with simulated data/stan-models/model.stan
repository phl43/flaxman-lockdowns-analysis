data {
  ...
  int reported_cases[N2,M]; // reported cases
  int reported_deaths[N2, M]; // reported deaths -- the rows with i > N contain -1 and should be ignored
  matrix[N2, M] f; // h * s
...
model {
  ...
  for(m in 1:M){
    reported_deaths[EpidemicStart[m]:N[m], m] ~ neg_binomial_2(E_deaths[EpidemicStart[m]:N[m], m], phi);
   }
}
