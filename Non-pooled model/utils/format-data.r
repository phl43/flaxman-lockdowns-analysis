...
make_data = function(out, filename,label,counterfactual="", i, days=7) {
  ...
  if(counterfactual != "") {
    out$E_deaths = out[[paste0("E_deaths", counterfactual)]] 
    estimated.deaths = out[[paste0("E_deaths", counterfactual)]] 
    out$prediction = out[[paste0("prediction", counterfactual)]]
  }
  out$E_deaths <- out$E_deaths[,1:N2,i]
  out$prediction = out$prediction[,1:N2,i]
  estimated.deaths <- estimated.deaths[,1:N2,i]
  # each column is the mean of the estimated number of deaths on that day across the posterior draws in country i
