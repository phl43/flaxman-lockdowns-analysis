...
parameters {
  ...
  real<lower=0> alpha_hier[M, P]; // sudo parameter for the hier term for alpha
  real<lower=0> gamma;
  real<lower=0> kappa;
  real<lower=0> y[M];
  real<lower=0> phi;
...
transformed parameters {
    ...
    
    {
      ...
      for(i in 1:P){
        for (m in 1:M) {
          alpha[i,m] = alpha_hier[m,i] - ( log(1.05) / 6.0 );
        }
      }
      for (m in 1:M){
        ...
        Rt[,m] = mu[m] * exp(-X[m][,1:6] * alpha[1:6,m]);
...
generated quantities {
    matrix[N2, M] Rt_cf = rep_matrix(0,N2,M);
    matrix[N2, M] Rt_cf_adj = Rt;
    ...
    
    {
      ...
      for (m in 1:M){
        ...
        if (m != 10) {
          Rt_cf[,m] = mu[m] * exp(-X[m][,5] * sum(append_row(alpha[2:4,10], alpha[6,10])));
        } else {
          Rt_cf[,m] = mu[m] * exp(-X[m][,1:6] * alpha[1:6,m]);
        }
        Rt_cf_adj[1:N0,m] = Rt_cf[1:N0,m];

        prediction0[1:N0,m] = rep_vector(y[m],N0);
        prediction1[1:N0,m] = rep_vector(y[m],N0);
        for (i in (N0+1):N2) {
          convolution0 = dot_product(sub_col(prediction0, 1, m, i-1), tail(SI_rev, i-1));
          cumm_sum0[i,m] = cumm_sum0[i-1,m] + prediction0[i-1,m];
          prediction0[i, m] = ((pop[m]-cumm_sum0[i,m]) / pop[m]) * mu[m] * convolution0;

          convolution1 = dot_product(sub_col(prediction1, 1, m, i-1), tail(SI_rev, i-1));
          cumm_sum1[i,m] = cumm_sum1[i-1,m] + prediction1[i-1,m];
          Rt_cf_adj[i,m] = ((pop[m]-cumm_sum1[i,m]) / pop[m]) * Rt_cf[i,m];
          prediction1[i, m] = Rt_cf_adj[i,m] * convolution1;
        }
        E_deaths0[1, m]= 1e-15 * prediction0[1,m];
        E_deaths1[1, m]= 1e-15 * prediction1[1,m];
        for (i in 2:N2){
          E_deaths0[i,m] = ifr_noise[m] * dot_product(sub_col(prediction0, 1, m, i-1), tail(f_rev[m], i-1));
          E_deaths1[i,m] = ifr_noise[m] * dot_product(sub_col(prediction1, 1, m, i-1), tail(f_rev[m], i-1));
        }
      }
    }
}
